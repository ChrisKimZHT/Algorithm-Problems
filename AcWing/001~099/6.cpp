...
void solve()
{
    ...
    for (int i = 1; i <= N; i++)
        cin >> v[i] >> w[i] >> s[i];
    vector<vector<int>> dp(N + 10, vector<int>(V + 10));
    for (int i = 1; i <= N; i++)
    {
        for (int r = 0; r < v[i]; r++)
        {
            ...
            {
                dp[i][k] = dp[i - 1][k];
                if (que.size() && que.front() < k - s[i] * v[i])
                    que.pop_front();
                while (que.size() && dp[i - 1][que.back()] - (que.back() - r) / v[i] * w[i] <= dp[i - 1][k] - (k - r) / v[i] * w[i])
                    que.pop_back();
                if (que.size())
                    dp[i][k] = max(dp[i][k], dp[i - 1][que.front()] + (k - que.front()) / v[i] * w[i]);
                que.push_back(k);
            }
        }
        // for (int j = 0; j <= V; j++)
